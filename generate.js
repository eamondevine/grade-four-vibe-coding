...
// ─── Config ───────────────────────────────────────────
const BASE_URL = 'https://eamondevine.github.io/grade-four-vibe-coding/students';
...
async function generate() {
  ...
  if (rows.length > 0) Object.keys(rows[0]).forEach(h => console.log(' ', JSON.stringify(h)));
  ...
  for (const row of rows) {
    ...
    if (seen.has(key)) {
      console.warn(`⚠️  Duplicate detected, skipping: ${name} (class ${row[COL_CLASS]}, seat ${row[COL_SEAT]})`);
      skipped++;
      continue;
    }
    seen.add(key);
